...
function ModalShell({ title, onClose, children, wide }) {
  return (
    <div className="fixed inset-0 z-40 flex items-start justify-center bg-black/40 p-4 overflow-y-auto" onClick={onClose}>
      <div
        className={`bg-white dark:bg-ink-800 rounded-2xl shadow-xl border border-slate-200 dark:border-ink-700 w-full ${wide ? 'max-w-3xl' : 'max-w-xl'} my-8`}
...
function SubmitModal({ onClose, onSubmitted, showToast }) {
  ...
  return (
    <ModalShell title="バグ報告・改善要望を送る" onClose={onClose} wide>
...
function DetailModal({ item, isAdmin, onClose, onChanged, showToast }) {
  ...
  return (
    <ModalShell title={`#${item.id} ${item.title}`} onClose={onClose} wide>
